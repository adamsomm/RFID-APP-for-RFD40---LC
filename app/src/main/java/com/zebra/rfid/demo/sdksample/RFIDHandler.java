...
class RFIDHandler implements IDcsSdkApiDelegate, Readers.RFIDReaderEventHandler {
    ...
    private final int DEVICE_STD_MODE = 0;
    private final int DEVICE_PREMIUM_PLUS_MODE = 1;
...
*/
    ...
    public synchronized void performInventory() {
        ...
        if (isInventoryRunning) {
            context.sendToast("Inventory already running.");
            return;
        }

        try {
            reader.Actions.Inventory.stop(); // clean slate
            Thread.sleep(100);               // allow settle time
        } catch (Exception e) {
            Log.w(TAG, "Stop before start failed (safe to ignore): " + e.getMessage());
        }

        try {
            synchronized (scannedTags) {
                scannedTags.clear(); // Clear previously scanned tags
            }
